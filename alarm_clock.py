...
class RoundedFrame(tk.Canvas):
    def __init__(self, parent, w, h, radius=20, bg=CARD):
...
class AlarmClock:
    ...
    def _build_ui(self):
        ...
        set_card = RoundedFrame(self.root, 300, 290)
        ...
        # ── 日期选择器 ──
        tk.Label(set_card, text="设置日期",
                 font=("Microsoft YaHei", 11), fg=SUB, bg=CARD).place(x=150, y=140, anchor="center")
        ...
        date_y = 190
        self._make_picker(set_card, 60,  date_y, "年", self.year_var, 2024, 2035)
        self._make_picker(set_card, 150, date_y, "月", self.month_var, 1, 12)
        self._make_picker(set_card, 240, date_y, "日", self.day_var, 1, 31)

        tk.Label(set_card, text="-", font=("Helvetica", 18, "bold"),
                 fg=SUB, bg=CARD).place(x=104, y=date_y-3)
        tk.Label(set_card, text="-", font=("Helvetica", 18, "bold"),
                 fg=SUB, bg=CARD).place(x=194, y=date_y-3)
    ...
    def _start_alarm(self):
        ...
        try:
            self.alarm_time = datetime.datetime(y, mo, d, h, m, s)
        except ValueError:
            messagebox.showwarning("日期无效", "请输入有效的日期")
            return
        ...
        self.status_label.config(
            text=f"●  已设置  {self.alarm_time.strftime('%Y-%m-%d %H:%M:%S')}",
            fg=GREEN)
    ...
    def _trigger(self):
        ...
        self._blink()
        messagebox.showinfo("闹钟", f"⏰ 时间到了！\n{self.alarm_time.strftime('%Y-%m-%d %H:%M:%S')}")
        self._stop_alarm()
